...
class Tool < ApplicationRecord
  ...
  # Single source of truth: name => default_price. All clients (API, mobile, web) use this.
  REPAIR_TOOLS_WITH_PRICES = {
    'No Works Required' => 0,
    '½ set epoxy resin' => 90,
  ...
  def self.common_tool_names
    REPAIR_TOOLS_WITH_PRICES.keys
  end

  def self.default_price_for_name(tool_name)
    return nil unless tool_name.present?

    REPAIR_TOOLS_WITH_PRICES[tool_name]
  end

  # For API: array of { name:, default_price: } so clients never hardcode prices.
  def self.repair_tools_for_api
    REPAIR_TOOLS_WITH_PRICES.map { |name, default_price| { name: name, default_price: default_price } }
  end
