...
module Api
  module V1
    class ImagesController < Api::V1::BaseController
      ...
      def find_work_order
        WorkOrder.find(params[:work_order_id] || params[:window_schedule_repair_id])
      end
      ...
      def upload_image_via_temp_window
        temp_window = @work_order.windows.build(location: 'Temporary')
      ...
      def handle_upload_error(error)
        Rails.logger.error "Window image upload error: #{error.message}"
      ...
      def purge_existing_images
        @work_order.images.purge if @work_order.images.attached?
      end

      def attach_new_images
        params[:images].each { |image| @work_order.images.attach(image) }
      end
      ...
      def build_upload_success_response
        {
          ...
          image_count: @work_order.images.count,
          image_urls: @work_order.images.map { |img| url_for(img) }
        }
      end
