...
RSpec.describe WorkOrders::DecisionService do
  ...
  describe '#call' do
    context 'when decision is accept' do
      it 'creates Decision, FreshBooks client and invoice and marks work order as approved' do
        ...
        expect(work_order.decision).to be_present
        expect(work_order.decision.decision).to eq('approved')
      end
    end

    context 'when decision is decline' do
      it 'creates Decision, marks work order as rejected and sends admin email' do
        allow(MailerSendEmailService).to receive(:new).and_return(double(call: true))
        ...
        expect(work_order.decision).to be_present
        expect(work_order.decision.decision).to eq('rejected')
      end
    end
  end
end
